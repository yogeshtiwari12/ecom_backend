...
export default function EnhancedDeliveryDashboard() {
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      case "high":
        return "text-pink-400 bg-pink-600/20 border-pink-500/50"
      case "medium":
  ...
  const getAllOrders = useMemo(() => {
    ...
    return userswithproduuct.flatMap((user: any, userIdx: number) =>
      (user.products || []).map((product: any, prodIdx: number) => ({
        ...
        productId: product.productId, // Added productId to the mapping
        ...
        distance: "N/A",
        rating: 4.5,
        amount: typeof product.productprice === "number" ? product.productprice : 0,
        paymentMethod: "Card",
        payment: null,
                            ...
                            {order.status !== "delivered" && order.status !== "cancelled" && (
                              <div className="mb-6">
                                ...
                                >
                                  ...
                                  {order.status === "pending" && <option value="delivered">Delivered</option>}
                                  {order.status === "picked_up" && (
                                    <>
                  ...
                  <tbody className="divide-y divide-purple-700/10">
                    {filteredOrders.map((order) => (
                      <tr key={order.id} className="hover:bg-slate-800/40 transition-colors duration-200 group">
                        ...
                        {/* <td className="p-4">
                          <span className="text-slate-400 text-sm">{order.payment === null ? "-" : order.payment}</span>
                        </td> */}
                        <td className="p-4">
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-2">
                            <span
                              className={`px-3 py-1.5 rounded-full text-xs font-medium border backdrop-blur-sm ${order.status === "pending"
